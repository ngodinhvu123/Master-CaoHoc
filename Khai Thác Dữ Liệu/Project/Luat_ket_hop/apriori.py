...
D = []
T = []
with open("DataSet7.txt", "r", encoding = "utf8") as f:
    for line in f:
        T = []
        transactions += 1
...
"""Compute frequent 1-itemset"""#tính phổ biến của 1 phần tử 
L1 = [] # Tính phổ biến  của 1 phần tử
for key in C1: #C1 Tập phổ biến có 1 phần tử
    if (100 * C1[key]/transactions) >= support:
        list = []
        list.append(key)
        L1.append(list)
print("----------------------FREQUENT 1-ITEMSET-------------------------")
print(L1)#In tập phổ biến của 1 phần tử
print("-----------------------------------------------------------------")
...
def frequent_itemsets():
    ...
    while Lk_1 != []:
        ...
        print(Lk)# in tập phổ biển mức độ sâu k
        print("------------------------------------------------------------------")
        for l in Lk:
            Lk_1.append(l)
        k += 1
